        ...
        <section className="mb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {["Platinum Tier", "Gold Tier", "Silver Tier"].map((tier, idx) => (
              <div
                key={idx}
                className="min-h-[220px] rounded-2xl border border-dashed border-white/15 bg-white/[0.01] p-6 flex flex-col items-center justify-center text-center text-slate-500 font-mono text-sm"
              >
        ...
        <section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
              <div
                key={i}
                className="h-28 rounded-2xl border border-dashed border-white/15 bg-white/[0.01] flex items-center justify-center text-slate-500 font-mono text-xs text-center p-4"
              >
                <span>[ Sponsor Logo {i} ]</span>
              </div>
            ))}
          </div>
        </section>
